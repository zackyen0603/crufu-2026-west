...
var SCENARIO_LEGS=[
 {leg:21,runner:"Eason",km:8.3},{leg:22,runner:"Eric.Lin",km:8.0},
 {leg:23,runner:"Eason",km:2.0,part:"23A"},{leg:23,runner:"Eric.Lin",km:2.0,part:"23B"},{leg:23,runner:"Zack",km:2.0,part:"23C"},{leg:23,runner:"Kevin",km:2.0,part:"23D"},
 {leg:24,runner:"Zack",km:9.2},{leg:25,runner:"Kevin",km:8.3},
...
function buildScenario(){
 ...
 var b=document.createElement("button");b.type="button";b.className="appTabBtn";b.setAttribute("data-tab","scenario");b.setAttribute("role","tab");b.setAttribute("aria-controls","tab-scenario");b.setAttribute("aria-selected","false");b.innerHTML="<strong>新方案推估</strong><small>Ailsa 8 km 四人分攤</small>";nav.appendChild(b);
 ...
 p.innerHTML='<div class="tabPanelHeader"><div><h2>新方案推估｜Ailsa 最後一輪不跑</h2><p class="muted">第 23 棒原 8.0 km 改由 Eason、Eric.Lin、Zack、Kevin 各跑 2.0 km；其餘棒次維持目前安排。第 21 棒以 02:30 官方管制統一起跑為錨點。</p></div><div class="sectionJump"><button id="scenarioRefresh" class="secondary" type="button">重新抓取共享資料</button></div></div><section class="dashKpis scenarioKpis"><div class="dashKpi"><span>方案起跑錨點</span><b id="scStart">—</b><small>第 21 棒</small></div><div class="dashKpi"><span>預估完賽</span><b id="scFinish">—</b><small>第 30 棒抵達</small></div><div class="dashKpi"><span>剩餘跑步時間</span><b id="scRemaining">—</b><small>不含新增等待</small></div><div class="dashKpi"><span>全程牆上時間</span><b id="scWall">—</b><small>06:52 起跑至預估完賽</small></div></section><section class="card"><h3>第 23 棒 8 km 分攤</h3><div class="scenarioSplit"><span>Eason 2 km</span><span>Eric.Lin 2 km</span><span>Zack 2 km</span><span>Kevin 2 km</span></div><p class="small muted">每段依目前共享基準配速推估；此頁只做時間模擬，不會更改正式 30 棒分配與實際紀錄。</p></section><section class="card" id="scenarioForecast"><h3>第 21–30 棒推估</h3><div class="tableWrap"><table><thead><tr><th>棒次</th><th>跑者</th><th>距離</th><th>採用配速</th><th>預估用時</th><th>預估開始</th><th>預估結束</th></tr></thead><tbody id="scenarioBody"></tbody></table></div></section><section class="card"><h3>假設</h3><p class="small muted">目前模型從第 21 棒 02:30 開始連續推算，未另外加入 21 棒之後可能發生的交接、交通或管制等待；若後續有新的實際抵達時間，共享資料更新後，此頁仍保留此方案供比較。</p><div id="scenarioStatus" class="small muted">讀取共享資料中…</div></section>';
 main.appendChild(p);
 var st=document.createElement("style");st.textContent='.scenarioSplit{display:flex;flex-wrap:wrap;gap:8px;margin:10px 0}.scenarioSplit span{padding:8px 12px;border:1px solid var(--line,#ddd);border-radius:999px;font-weight:700}.scenarioKpis{margin-top:0}@media(max-width:700px){.scenarioKpis{grid-template-columns:repeat(2,minmax(0,1fr))}}';document.head.appendChild(st);
...
function renderScenario(){
 ...
 tbody.innerHTML="";
 SCENARIO_LEGS.forEach(function(x){var pv=paces[x.runner]||"07:00",dur=minsFromPace(pv)*x.km,st=new Date(t),en=addMin(st,dur);total+=dur;t=en;var tr=document.createElement("tr");var label=x.part?x.part:"#"+x.leg;tr.innerHTML="<td>"+label+"</td><td><b>"+x.runner+"</b></td><td>"+x.km.toFixed(1)+" km</td><td>"+fmtPace(pv)+"</td><td>"+fmtDur(dur)+"</td><td>"+fmtTime(st)+"</td><td><b>"+fmtTime(en)+"</b></td>";tbody.appendChild(tr)});
